...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            output = model(images)
            output_probs = nn.functional.softmax(output, 1)
            entropy = torch.gather(output_probs, 1, target.unsqueeze(1))
            l_entropy.extend(entropy)
            ...
            if i % 10 == 0:
                progress.display(i)
        l_entropy = [i[0].item() for i in l_entropy]
        print(' * Acc@1 {top1.avg:.3f}'.format(top1=top1))

    return top1.avg, l_entropy
...
if __name__ == "__main__":
    ...
    inf_timesteps = [9, 10, 11, 12, 13, 14, 15]
    ...
    for length in lengths:
        ...
        for timestep in inf_timesteps:
            ...
            with torch.no_grad():
                ...
                acc1, curr_conf = validate(val_loader, model, criterion)
                results[length][cfg_name][timestep] = (acc1.item(), np.mean(curr_conf))
                print(results)
